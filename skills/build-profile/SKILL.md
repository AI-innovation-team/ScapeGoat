...
| 模式 | 画像对象 | 适用 | 协议文件 |
|---|---|---|---|
| **A. 知情人访谈** | 用户描述的**第三者** | "帮我分析某人"、用户提供对他人的观察 | `${CLAUDE_PLUGIN_ROOT}/src/scapegoat/prompts/profile/init.md` |
| **B. 心理测评** | **作答者本人** | "给我做个测评"、画像对象自己在场作答 | `${CLAUDE_PLUGIN_ROOT}/src/scapegoat/prompts/profile/assessment.md` |

## Required reading

1. `${CLAUDE_PLUGIN_ROOT}/src/scapegoat/prompts/profile/merge_rules.md` — 写入契约，两种模式共用。
...
4. 校验：`scapegoat profile budget <subject_id> --base-dir <base_dir> --strict`（或 MCP `profile_budget`），超预算压缩至通过。调用方式见 [cli-invocation.md](../cli-invocation.md)。
