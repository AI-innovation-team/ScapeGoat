...
- **没给 → 自动扫描候选**，两处都要扫，合并后列出：
  1. **用户自己的画像**：Glob `profiles/*/profile.md`（若空，再扫当前目录 `*/profile.md`）。
  2. **插件自带的示例画像**：Glob `${CLAUDE_PLUGIN_ROOT}/personas/*/profile.md`。装完插件即可用，新目录里也能立刻召唤。

  同名时以用户自己的为准（他自己建的画像优先于示例）。列出时标明来源：

  > 没说请谁上身。当前可召唤：
  > - 你的画像：`<扫到的>`
  > - 内置示例：`luoyonghao`、`mentor`、`student`
  >
  > 选哪个？（或直接给我一个 `profile_dir`）

- 一个都没扫到（连内置的都没有，说明不是通过插件安装的）→ 告知情况，请用户提供 `profile_dir`。
