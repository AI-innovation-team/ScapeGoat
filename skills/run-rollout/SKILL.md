...
1. Ensure both profiles are frozen (use **scapegoat:freeze-profile** if not).
   Profiles come from the user's `profiles/` or, for a quick demo with nothing
   set up, the bundled `${CLAUDE_PLUGIN_ROOT}/personas/` (`student` as generator
   against `mentor` as discriminator is the ready-made pair).
