...
def test_repo_tree_is_in_sync():
    """The committed tree must already agree with the identity it records.

    Read that identity rather than hardcoding it, so a transfer or rename does
    not fail this test for the wrong reason.
    """

    repo = sync_repo_urls.recorded_repo()
    assert repo is not None
    mcp = Path(sync_repo_urls.ROOT / ".mcp.json").read_text(encoding="utf-8")
    visibility = "public" if "git+https" in mcp else "private"
    assert sync_repo_urls.sync(repo, visibility, check=True) == 0
